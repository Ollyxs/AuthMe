...
class UserCrudTestCase(unittest.TestCase):
    ...
    def test_get_user(self):
        user = UserModel(first_name='jane', email='jane@example.com', password='testpass', last_name="asd", role="admin")
        db.session.add(user)
        db.session.commit()
        ...
        self.assertEqual(response.json['password'], user.password)
        # self.assertFalse('password' in response.json)
        self.assertFalse('role' in response.json)

    def test_update_user(self):
        user = UserModel(first_name='jane', email='jane@example.com', password='testpass', role='admin', last_name="asd")
        db.session.add(user)
        db.session.commit()
        user_data = {
        ...
        self.assertEqual(response.json['email'], user_data['email'])
        self.assertEqual(response.json['last_name'], user_data['last_name'])
        self.assertEqual(response.json['password'], user_data['password'])
        # self.assertFalse('password' in response.json)
        updated_user = UserModel.query.get(user.id)
        self.assertEqual(updated_user.first_name, user_data['first_name'])
        self.assertEqual(updated_user.last_name, user_data['last_name'])
        self.assertEqual(updated_user.email, user_data['email'])

    def test_delete_user(self):
        user = UserModel(first_name='jane_updated', email='jane_updated@example.com', password='testpass', last_name='asd', role='admin')
        db.session.add(user)
        db.session.commit()
